...
#define TRANS_ELEM_TYPE(in, out, lda, ldb, type_t)                             \
  {                                                                            \
    ...
    for (ii = 0; ii + 7 < lda; ii += 8) {                                      \
      for (jj = 0; jj < ldb; jj++) {                                           \
        for (kk = 0; kk < 8; kk++) {                                           \
          out_type[jj * lda + ii + kk] = in_type[ii * ldb + kk * ldb + jj];    \
        }                                                                      \
      }                                                                        \
    }                                                                          \
...
int64_t bshuf_trans_byte_elem_remainder(const void *in, void *out,
                                        const size_t size,
                                        const size_t elem_size,
                                        const size_t start) {
...
int64_t bshuf_trans_bit_byte_remainder(const void *in, void *out,
                                       const size_t size,
                                       const size_t elem_size,
 ...
 * the bytes. */
int64_t bshuf_trans_byte_bitrow_scal(const void *in, void *out,
                                     const size_t size,
                                     const size_t elem_size) {
...
int64_t bshuf_untrans_bit_elem_scal(const void *in, void *out,
...
int64_t bshuf_trans_byte_elem_SSE_16(const void *in, void *out,
                                     const size_t size) {
  ...
  return bshuf_trans_byte_elem_remainder(in, out, size, 2, size - size % 16);
}

/* Transpose bytes within elements for 32 bit elements. */
int64_t bshuf_trans_byte_elem_SSE_32(const void *in, void *out,
                                     const size_t size) {
  ...
  return bshuf_trans_byte_elem_remainder(in, out, size, 4, size - size % 16);
}

/* Transpose bytes within elements for 64 bit elements. */
int64_t bshuf_trans_byte_elem_SSE_64(const void *in, void *out,
                                     const size_t size) {
  ...
  return bshuf_trans_byte_elem_remainder(in, out, size, 8, size - size % 16);
}
...
/* For data organized into a row for each bit (8 * elem_size rows), transpose
 * the bytes. */
int64_t bshuf_trans_byte_bitrow_SSE(const void *in, void *out,
                                    const size_t size, const size_t elem_size) {
...
/* Shuffle bits within the bytes of eight element blocks. */
int64_t bshuf_shuffle_bit_eightelem_SSE(const void *in, void *out,
                                        const size_t size,
                                        const size_t elem_size) {
...
/* For data organized into a row for each bit (8 * elem_size rows), transpose
 * the bytes. */
int64_t bshuf_trans_byte_bitrow_AVX(const void *in, void *out,
                                    const size_t size, const size_t elem_size) {
  ...
  if (elem_size % 4)
    return bshuf_trans_byte_bitrow_SSE(in, out, size, elem_size);
  ...
  for (jj = 0; jj + 31 < nbyte_row; jj += 32) {
    for (ii = 0; ii + 3 < elem_size; ii += 4) {
      for (hh = 0; hh < 4; hh++) {
        ...
        for (kk = 0; kk < 8; kk++) {
          ymm_0[kk] = _mm256_loadu_si256(
              (__m256i *)&in_b[(ii * 8 + hh * 8 + kk) * nbyte_row + jj]);
        }

        for (kk = 0; kk < 4; kk++) {
          ymm_1[kk] = _mm256_unpacklo_epi8(ymm_0[kk * 2], ymm_0[kk * 2 + 1]);
          ymm_1[kk + 4] =
              _mm256_unpackhi_epi8(ymm_0[kk * 2], ymm_0[kk * 2 + 1]);
        }
        ...
        for (kk = 0; kk < 4; kk++) {
          ymm_1[kk * 2] =
              _mm256_unpacklo_epi32(ymm_0[kk * 2], ymm_0[kk * 2 + 1]);
          ymm_1[kk * 2 + 1] =
              _mm256_unpackhi_epi32(ymm_0[kk * 2], ymm_0[kk * 2 + 1]);
        }
      ...
      for (mm = 0; mm < 8; mm++) {
        ...
        _mm256_storeu_si256(
            (__m256i *)&out_b[(jj + mm * 2 + 0 * 16) * nrows + ii * 8],
            ymm_0[0]);
        _mm256_storeu_si256(
            (__m256i *)&out_b[(jj + mm * 2 + 0 * 16 + 1) * nrows + ii * 8],
            ymm_0[1]);
        _mm256_storeu_si256(
            (__m256i *)&out_b[(jj + mm * 2 + 1 * 16) * nrows + ii * 8],
            ymm_0[2]);
        _mm256_storeu_si256(
            (__m256i *)&out_b[(jj + mm * 2 + 1 * 16 + 1) * nrows + ii * 8],
            ymm_0[3]);
      }
    }
  }
...
/* Shuffle bits within the bytes of eight element blocks. */
int64_t bshuf_shuffle_bit_eightelem_AVX(const void *in, void *out,
                                        const size_t size,
                                        const size_t elem_size) {
...
#if defined(_OPENMP)
#pragma omp parallel for schedule(dynamic, 1) private(count)                   \
    reduction(+ : cum_count)
#endif
...
int64_t bshuf_bitshuffle_block(ioc_chain *C_ptr, const size_t size,
                               const size_t elem_size) {
...
int64_t bshuf_bitunshuffle_block(ioc_chain *C_ptr, const size_t size,
                                 const size_t elem_size) {
